...
from auditron.core.core import SavableMeta
from auditron.settings import settings
...
def load_plugins():
    auditron_tests_module = "auditron.testing.tests"
    if auditron_tests_module not in sys.modules:
        importlib.import_module(auditron_tests_module)
    else:
        importlib.reload(sys.modules[auditron_tests_module])

    auditron_functions_module = "auditron.functions"
    if auditron_functions_module not in sys.modules:
        importlib.import_module(auditron_functions_module)
    else:
        importlib.reload(sys.modules[auditron_functions_module])
...
def create_module_name(import_path: Path, root: Optional[Path]):
    if root is None:
        return "auditron_plugins"
    else:
        return "auditron_plugins." + ".".join(import_path.with_suffix("").relative_to(root).parts)
...
tests_registry = auditronTestRegistry()
