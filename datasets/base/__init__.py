...
from auditron.client.io_utils import compress, save_df
from auditron.client.python_utils import warning
from auditron.core.core import NOT_GIVEN, DatasetMeta, NotGivenOr, SupportedColumnTypes
from auditron.core.errors import auditronImportError
from auditron.core.validation import configured_validate_arguments
from auditron.registry.slicing_function import SlicingFunction, SlicingFunctionType
from auditron.registry.transformation_function import (
...
class Dataset(ColumnMetadataMixin):
    ...
    etc.). The prediction function that you wrap with the auditron `Model` should be able to
    ...
    def __init__(
        ...
        if validation:
            from auditron.core.dataset_validation import validate_dataset
        ...
        if validation:
            from auditron.core.dataset_validation import validate_column_types

            validate_column_types(self)

        if validation:
            from auditron.core.dataset_validation import (
        ...
        if validation:
            logger.info("Your 'pandas.DataFrame' is successfully wrapped by auditron's 'Dataset' wrapper class.")
    ...
    def load(cls, local_path: str):
        # load metadata
        with open(Path(local_path) / "auditron-dataset-meta.yaml", "r", encoding="utf-8") as meta_f:
    ...
    def save(self, local_path: str):
        with (open(Path(local_path) / "data.csv.zst", "wb") as f,):
            ...
            with open(Path(local_path) / "auditron-dataset-meta.yaml", "w", encoding="utf-8") as meta_f:
                yaml.dump(
                    {
                        "id": str(self.id),
                        "name": self.meta.name,
    ...
    def __str__(self) -> str:
        ...
        return super().__str__()  # default to `<auditron.datasets.base.Dataset object at ...>`
