...
def _get_plugin_method_full_name(func):
    from auditron.registry.registry import plugins_root

    path_parts = list(Path(inspect.getfile(func)).relative_to(plugins_root).with_suffix("").parts)
    path_parts.insert(0, "auditron_plugins")
...
class CallableMeta(SavableMeta, ABC):
    ...
    def __init__(
        ...
        if callable_obj:
            from auditron.registry.registry import get_object_uuid
    ...
    def populate_tags(self, tags=None):
        ...
        if self.name == "<lambda>":
            tags.append("lambda")
        elif self.full_name.partition(".")[0] == "auditron":
            tags.append("auditron")
        else:
            tags.append("custom")
        return tags
