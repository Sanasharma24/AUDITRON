...
from auditron.core.errors import auditronImportError
from auditron.core.savable import Artifact
from auditron.core.test_result import TestMessage, TestMessageLevel, TestResult
from auditron.datasets.base import Dataset
from auditron.exceptions.IllegalArgumentError import IllegalArgumentError
from auditron.models.base import BaseModel
from auditron.registry.auditron_test import auditronTest, auditronTestMethod, Test
from auditron.registry.registry import tests_registry
from auditron.registry.slicing_function import SlicingFunction
from auditron.registry.transformation_function import TransformationFunction
...
class TestSuiteResult:
    ...
    def to_mlflow(self, mlflow_client: MlflowClient = None, mlflow_run_id: str = None):
        ...
        from auditron.integrations.mlflow.auditron_evaluator_utils import process_text
    ...
    def to_wandb(self, run: Optional["wandb.wandb_sdk.wandb_run.Run"] = None) -> None:  # noqa
        ...
        try:
            ...
            raise auditronImportError("wandb") from e
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "repository for further assistance: https://github.com/auditron-AI/auditron."
            ) from e
...
class TestPartial:
    ...
    def _to_json(self, folder: Path, saved_uuid_status: Dict[str, bool]):
        params = dict(
            {
                pname: _build_test_input_json(
                    ...
                    p,
                    pname,
                    self.auditron_test.meta.args[pname].type,
                    saved_uuid_status,
                )
                for pname, p in self.provided_inputs.items()
                if pname in self.auditron_test.meta.args
            }
        )
        ...
        kwargs_params = [
            ...
            if pname not in self.auditron_test.meta.args
        ]
        if len(kwargs_params) > 0:
            params["kwargs"] = {"name": "kwargs", "value": "\n".join(kwargs_params), "type": "Kwargs"}

        if self.auditron_test.meta.uuid not in saved_uuid_status:
            test_folder = folder / str(self.auditron_test.meta.uuid)
            test_folder.mkdir(exist_ok=True)
            self.auditron_test.save(test_folder)

        return {
            "id": self.suite_test_id,
            "testUuid": str(self.auditron_test.meta.uuid),
...
def generate_test_partial(
    ...
    **params,
    ...
    if isinstance(test_fn, auditronTestMethod):
        actual_params = {k: v for k, v in test_fn.params.items() if v is not None}
    elif isinstance(test_fn, auditronTest):
        actual_params = {
        ...
        actual_params = dict()
        test_fn = auditronTestMethod(test_fn)
...
class Suite:
    ...
    methods to add new tests, execute all tests, and save the suite to a auditron instance.
    """
    ...
    def run(self, verbose: bool = True, **suite_run_args):
        ...
        **suite_run_args : Optional[dict]
        ...
        for test_partial in self.tests:
            ...
            try:
                result = test_partial.auditron_test(**test_params).execute()
    ...
    def to_unittest(self, **suite_gen_args) -> List[TestPartial]:
        ...
        **suite_run_args : Optional[dict]
        ...
        for test_partial in self.tests:
            test_params = self.create_test_params(test_partial, run_args)
            unittest: TestPartial = test_partial.auditron_test(**test_params)
            params_str = ", ".join(
    ...
    def create_test_params(test_partial, kwargs) -> TestParams:
        if isinstance(test_partial.auditron_test, auditronTestMethod):
            available_params = inspect.signature(test_partial.auditron_test.test_fn).parameters.items()
        else:
            available_params = inspect.signature(test_partial.auditron_test.__init__).parameters.items()
    ...
    def add_test(
        ...
        **params,
        ...
        ----------
        test_fn : Test
            A test method that will be executed or an instance of a auditronTest class.
        ...
        **params :
            ...
            This parameter will be ignored if `test_fn` is an instance of auditronTest.
    ...
    def upgrade_test(
        self, test: auditronTest, migrate_params_fn: Optional[Callable[[Dict[str, Any]], Dict[str, Any]]] = None
    ) -> "Suite":
        ...
        for test_to_upgrade in self.tests:
            if test_to_upgrade.auditron_test.display_name != test.display_name:
                continue

            test_to_upgrade.auditron_test = test
    ...
    @remove_test.register
    def _remove_test_by_reference(self, auditron_test: auditronTest):
        self.tests = [test for test in self.tests if test.auditron_test.meta.uuid != auditron_test.meta.uuid]
        return self
    ...
    def update_test_params(self, index: int, **params):
        ...
        **params :
        ...
        self.tests[index] = generate_test_partial(test.auditron_test, test.test_id, **inputs)
    ...
    def find_required_params(self):
        ...
        for test_partial in self.tests:
            if isinstance(test_partial.auditron_test, auditronTestMethod):
                available_params = inspect.signature(test_partial.auditron_test.test_fn).parameters.values()
            else:
                available_params = inspect.signature(test_partial.auditron_test.__init__).parameters.values()
                ...
                if p.default == inspect.Signature.empty:
                    if p.name not in test_partial.provided_inputs:
                        ...
                        if test_partial.provided_inputs[p.name].type != p.annotation:
                            raise ValueError(
                                f"Test {test_partial.auditron_test.func.__name__} requires {p.name} input to "
    ...
    def generate_tests(self, inputs: List[SuiteInput]):
        auditron_tests = [
            ...
            if contains_tag(test, "auditron") and not self._contains_test(test)
        ]

        for test in auditron_tests:
    ...
    def _contains_test(self, test: TestFunctionMeta):
        return any(t.auditron_test == test for t in self.tests)
    ...
    def load(cls, folder: str) -> "Suite":
        ...
        for test_json in suite_json.get("tests", []):
            ...
            test = auditronTest.load(test_folder, test_uuid, auditronTest._load_meta_locally(test_folder, test_uuid))
