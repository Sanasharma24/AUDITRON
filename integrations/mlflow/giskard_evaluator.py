...
from .evaluation_artifacts import (
    auditronDatasetEvaluationArtifact,
    auditronScanResultEvaluationArtifact,
    auditronScanSummaryEvaluationArtifact,
)
from .auditron_evaluator_utils import setup_dataset, setup_model, setup_scan

logger = logging.getLogger(__name__)


class auditronEvaluator(ModelEvaluator):
    ...
    def _wrap_dataset(self, dataset):
        if not isinstance(dataset.features_data, pd.DataFrame):
            raise ValueError("Only pd.DataFrame are currently supported by the auditron evaluator.")
        else:
            try:
                auditron_dataset = setup_dataset(dataset, self.evaluator_config)

                # log dataset
                local_path = auditron_dataset.to_mlflow(mlflow_client=self.client, mlflow_run_id=self.run_id)
                return auditron_dataset, local_path
    ...
    def _wrap_model(self, model, model_type, feature_names):
        try:
            auditron_model = setup_model(model, model_type, feature_names, self.evaluator_config)
            return auditron_model
        except Exception as e:
            analytics.track(
    ...
    def _perform_scan(self, auditron_model, auditron_dataset):
        try:
            scan_results = setup_scan(auditron_model, auditron_dataset, self.evaluator_config)
    ...
    def evaluate(
        ...
        # Wrapping dataset
        auditron_dataset, dataset_artifact_name = self._wrap_dataset(dataset)
        artifacts["auditron_dataset"] = auditronDatasetEvaluationArtifact(
            uri=get_artifact_uri(self.run_id, dataset_artifact_name), content=auditron_dataset.df
        )
        properties.update({"dataset_id": str(auditron_dataset.id)} if auditron_dataset is not None else "none")

        # Wrapping model
        auditron_model = self._wrap_model(model, model_type=model_type, feature_names=dataset.feature_names)
        properties.update({"model_class": fullname(auditron_model), "model_inner_class": fullname(auditron_model.model)})
        analytics.track("mlflow_integration", properties)

        # Perform scan
        scan_results, scan_artifact_names = self._perform_scan(auditron_model, auditron_dataset)
        ...
        artifacts["auditron_scan_results"] = auditronScanResultEvaluationArtifact(
            uri=get_artifact_uri(self.run_id, scan_results_artifact_name), content=scan_results
        )
        if scan_summary_artifact_name:
            artifacts["auditron_scan_summary"] = auditronScanSummaryEvaluationArtifact(
        ...
        print(
            "The evaluation with auditron ran successfully! You can now visualise the results by running 'mlflow ui' "
            "in the terminal."
        )
