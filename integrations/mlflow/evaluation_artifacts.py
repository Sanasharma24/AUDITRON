...
from auditron.client.io_utils import save_df


class auditronDatasetEvaluationArtifact(EvaluationArtifact):
...
class auditronScanResultEvaluationArtifact(EvaluationArtifact):
    def _save(self, output_artifact_path):
...
class auditronScanSummaryEvaluationArtifact(auditronDatasetEvaluationArtifact):
    def _load_content_from_file(self, local_artifact_path):
        with open(local_artifact_path, "rb") as ds_stream:
            return pd.read_csv(
                ZstdDecompressor().stream_reader(ds_stream),
