...
from auditron.core.core import SupportedModelTypes
from auditron.datasets import Dataset
from auditron.models import Model
from auditron.scanner import scan
...
def setup_dataset(dataset, evaluator_config):
    ...
    raise ValueError(
        ...
        "Make sure to pass only the attributes of auditron.Dataset "
        "(see https://docs.auditron.ai/en/stable/reference/datasets)."
    )
...
def setup_model(model, model_type, feature_names, evaluator_config):
    ...
    if hasattr(model, "predict"):
        ...
        raise ValueError(
            "The model types supported by the auditron evaluator are: "
    ...
    raise ValueError(
        ...
        "Make sure to pass only the attributes of auditron.Model "
        "(see https://docs.auditron.ai/en/stable/reference/models)."
    )


def setup_scan(auditron_model, auditron_dataset, evaluator_config):
    scan_config = evaluator_config.get("scan_config", None)
    if scan_config is None:
        return scan(model=auditron_model, dataset=auditron_dataset)

    config_set = set(scan_config.keys())
    sign = inspect.signature(scan)
    sign_set = set(sign.parameters.keys())
    if config_set.issubset(sign_set):
        return scan(model=auditron_model, dataset=auditron_dataset, **scan_config)

    raise ValueError(
        f"The provided parameters {config_set - sign_set} in scan_config are not valid. "
        "Make sure to pass only the attributes of auditron.scan."
    )
