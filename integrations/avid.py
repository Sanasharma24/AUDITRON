...
from ..core.errors import auditronImportError
...
try:
    ...
    raise auditronImportError("Avidtools is not installed. Please install it with `pip install avidtools`.")
...
def create_report_from_issue(issue: Issue, model: BaseModel, dataset: Dataset = None) -> Report:
    ...
    references = [Reference(type="source", label="auditron Scanner", url="https://auditron.ai")]
    ...
    report.description = LangValue(
        lang="eng", value=f"The model was evaluated by the auditron Scanner {auditron.__version__}."
    )
    report.problemtype = Problemtype(
        ...
        credit=[LangValue(lang="eng", value="auditron")],
    )
    if issue.meta.get("metric"):
        ...
        report.metrics = [
            Metric(
                ...
                detection_method=Detection(type=MethodEnum.thres, name=f"auditron Scanner {auditron.__version__}"),
