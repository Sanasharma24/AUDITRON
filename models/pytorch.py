...
class PyTorchModel(MLFlowSerializableModel):
    def __init__(
        ...
        **kwargs,
        ...
        This class provides a default wrapper around the PyTorch library for usage with auditron.
    ...
    def save_pytorch_meta(self, local_path, *_args, **_kwargs):
        with open(Path(local_path) / "auditron-model-pytorch-meta.yaml", "w", encoding="utf-8") as f:
            yaml.dump(
                {
                    "device": self.device,
    ...
    def load_pytorch_meta(cls, local_dir):
        pytorch_meta_file = Path(local_dir) / "auditron-model-pytorch-meta.yaml"
    ...
    def to_mlflow(self, artifact_path: str = "pytorch-model-from-auditron", **kwargs):
