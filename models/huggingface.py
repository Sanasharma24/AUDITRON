...
used with auditron. It supports both standard models from the ``transfomers``
...
Now we can wrap the model with auditron:

>>> import auditron as gsk
>>> gsk_model = gsk.Model(
...     model=model,
...
>>> type(gsk_model)
<class 'auditron.models.huggingface.HuggingFaceModel'>

Notice how using :class:`auditron.Model` automatically detected that we are using
a HuggingFace model and returned the correct subclass. You can also provide the
class explicitly using :class:`auditron.models.huggingface.HuggingFaceModel`.
...
out-of-memory errors::
    ...
    import time
    import auditron
    from auditron.models.huggingface import HuggingFaceModel

    wrapped_model = HuggingFaceModel(...)
    wrapped_dataset = auditron.Dataset(...)

    with auditron.models.cache.no_cache():
...
An important thing to notice is that auditron expects classification models to
...
from auditron.core.core import ModelType
from auditron.core.validation import configured_validate_arguments
from auditron.models.base import WrapperModel
...
class HuggingFaceModel(WrapperModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        ``transformers`` library for usage with auditron.
        ...
        name : Optional[str]
            The name of the model, used in the auditron UI.
    ...
    def load_model(cls, local_path, model_py_ver: Optional[Tuple[str, str, str]] = None, *args, **kwargs):
        huggingface_meta_file = Path(local_path) / "auditron-model-huggingface-meta.yaml"
    ...
    def save_huggingface_meta(self, local_path, *args, **kwargs):
        with open(Path(local_path) / "auditron-model-huggingface-meta.yaml", "w", encoding="utf-8") as f:
            yaml.dump(
                {
    ...
    def model_predict(self, data):
        ...
        if self.is_classification and hasattr(predictions, "logits"):
            ...
            if self.model_postprocessing_function:
                ...
                logger.warning(
                    "Your model output is logits. In auditron, we expect the output to be probabilities."
    ...
    def to_mlflow(self, artifact_path: str = "transformers-model-from-auditron", **kwargs):
        import mlflow
