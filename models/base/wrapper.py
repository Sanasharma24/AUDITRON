...
class WrapperModel(BaseModel, ABC):
    ...
    def save_data_preprocessing_function(self, local_path: Union[str, Path], *_args, **_kwargs):
        with open(Path(local_path) / "auditron-data-preprocessing-function.pkl", "wb") as f:
    ...
    def save_model_postprocessing_function(self, local_path: Union[str, Path], *_args, **_kwargs):
        with open(Path(local_path) / "auditron-model-postprocessing-function.pkl", "wb") as f:
    ...
    def save_wrapper_meta(self, local_path, *_args, **_kwargs):
        with open(Path(local_path) / "auditron-model-wrapper-meta.yaml", "w", encoding="utf-8") as f:
            yaml.dump(
                {
    ...
    def load_data_preprocessing_function(cls, local_path: Union[str, Path], *_args, **_kwargs):
        ...
        file_path = local_path / "auditron-data-preprocessing-function.pkl"
        if file_path.exists():
    ...
    def load_model_postprocessing_function(cls, local_path: Union[str, Path], *_args, **_kwargs):
        local_path = Path(local_path)
        file_path = local_path / "auditron-model-postprocessing-function.pkl"
        if file_path.exists():
            with open(file_path, "rb") as f:
                try:
                    # According to https://github.com/cloudpipe/cloudpickle#cloudpickle:
    ...
    def load_wrapper_meta(cls, local_dir, *args, **kwargs):
        wrapper_meta_file = Path(local_dir) / "auditron-model-wrapper-meta.yaml"
    ...
    def to_mlflow(self, artifact_path: str = "prediction-function-from-auditron", *_args, **_kwargs):
        import mlflow

        def _auditron_predict(df):
            return self.predict(df)

        class MLflowModel(mlflow.pyfunc.PythonModel):
            def predict(self, df):
                return _auditron_predict(df)
