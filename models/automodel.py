...
_ml_libraries = {
    ("auditron.models.huggingface", "HuggingFaceModel"): [("transformers", "PreTrainedModel")],
    ("auditron.models.sklearn", "SKLearnModel"): [("sklearn.base", "BaseEstimator")],
    ("auditron.models.langchain", "LangchainModel"): [("langchain.chains.base", "Chain")],
    ("auditron.models.catboost", "CatboostModel"): [("catboost", "CatBoost")],
    ("auditron.models.pytorch", "PyTorchModel"): [("torch.nn", "Module")],
    ("auditron.models.tensorflow", "TensorFlowModel"): [("tensorflow", "Module"), ("keras", "Model")],
}
...
def _infer_auditron_cls(model: Any):
    if inspect.isfunction(model) or inspect.ismethod(model):
        return PredictionFunctionModel
    else:
        for _auditron_class, _base_libs in _ml_libraries.items():
            try:
                auditron_cls = _get_class(*_auditron_class)
                base_libs = [_get_class(*_base_lib) for _base_lib in _base_libs]
                if isinstance(model, tuple(base_libs)):
                    return auditron_cls
            except AttributeError:
                logger.warning("Error while getting auditron model & associated framework", exc_info=1)
                pass
            except ImportError:
                pass
    return None
...
class Model(CloudpickleSerializableModel):
    ...
    ----------
    model : Any
        Could be any function or ML model. The standard model output required for auditron is:
    ...
    **kwargs :
    ...
    def __new__(
        ...
        **kwargs,
        ...
        if not model:
            ...
            auditron_cls = _infer_auditron_cls(model)
            ...
            # if auditron_cls == None -> get the methods from CloudpickleSerializableModel
            is_overriden = cls.__name__ != "Model"  # TODO: Improve this
            if is_overriden:
                if not auditron_cls:
                    auditron_cls = CloudpickleSerializableModel
                ...
                input_type = "'prediction_function'" if auditron_cls == PredictionFunctionModel else "'model'"
                logger.info(
                    "Your "
                    + input_type
                    + " is successfully wrapped by auditron's '"
                    + str(auditron_cls.__name__)
                    + "' wrapper class."
                )
                possibly_overriden_cls = auditron_cls
                ...
                raise NotImplementedError(
                    ...
                    "https://auditron.readthedocs.io/en/latest/getting-started/scan"
                )

            methods = dict(possibly_overriden_cls.__dict__)
            output_cls = type(possibly_overriden_cls.__name__, (auditron_cls,), methods)
    ...
    def load(cls, local_dir, model_py_ver: Optional[Tuple[str, str, str]] = None, *args, **kwargs):
        """
        Load a auditron model from the specified local directory.
